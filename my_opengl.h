...
public:
    ...
    void set_Y_move_to_bottom(float flo)
    {
        y_move_to_bottom+=flo;
    }
    ...
    void check_supperts(const float &x,const float &y,QVector<int>&ares);
signals:
...
private:
    ...
    Transform3D normal_change;           //计算是否需要支撑时使用的变换矩阵,用于计算变换后的法向量
